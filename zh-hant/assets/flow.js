...
(function () {
  ...
  /* 滾動期間讓出主線程：這是最底層背景，滾動時靜止一瞬肉眼無感，
     卻能把每一幀都讓給滾動合成——配合 ~30fps 限幀，根治「滾動還有點卡」。 */
  ...
  function breathe(now) {
    ...
    if (now - lastScroll < 220) return;   /* 滾動進行中：跳過這一幀重繪 */
    if (now - lastDraw < 30) return;      /* 限到約 30fps：緩慢的背景無需 60fps */
    lastDraw = now;
    chi += 0.0011;
